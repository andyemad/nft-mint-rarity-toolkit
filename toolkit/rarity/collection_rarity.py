#!/usr/bin/env python3
"""the rarity-test collection rarity engine v2 (OpenSea API source).

Pulls all 5000 the rarity-test collection token traits via OpenSea v2 API (cursor pagination),
...
Usage:
  python3 raritytest_rarity.py fetch     # pull all traits -> traits.json
  python3 raritytest_rarity.py rank      # compute rarity if traits.json exists
  python3 raritytest_rarity.py both      # fetch then rank (default)
  python3 raritytest_rarity.py mine      # your holdings, ranked
"""
...
def mine():
    ...
    rarest = held[:1]
    print(f"YOUR {len(held)} the rarity-test collection holdings (rarest first). Avg rank {avg:.0f}/5000:")
    for tid in held:
