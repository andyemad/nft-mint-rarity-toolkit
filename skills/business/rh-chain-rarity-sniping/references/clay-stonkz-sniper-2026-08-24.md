# Clay StonKz reveal-sniper daemon (built 2026-08-24)

Standalone guarded auto-buy daemon at
`~/Projects/bunker-snipe/clay_sniper.py`. Rehearsed end-to-end in DRY mode;
buy path proven via eth_call against a live claystonkz order. ARMED same day
with Emad's explicit approval slate (max 3 buys, ≤0.010/buy, ≤0.030/day,
0.050 reserve); runs as a Hermes background process polling every 20s.
Launch interpreter matters: use `~/Projects/bunker-snipe/.venv/bin/python`
(system python3 lacks eth_account and the daemon dies instantly with
ModuleNotFoundError while *looking* started).

Live visual companion: `~/Projects/bunker-snipe/sniper_status_server.py`
...
This was built 8/24 as a quick throwaway. **Emad then corrected the delivery:
Mint Room is the all-in-one suite — integrate the status there, don't run a
separate probe.** See "Integrating a module into Mint Room" below. The 8139
...
1. **Reveal detection** — eth_call `tokenURI(uint256)` on ~8 spread tokens;
   ...
   as `~/.hermes/scripts/claystonkz_reveal_watch.py`.
2. **Rank** — FAST PATH first: `GET /api/v2/chain/robinhood/contract/{CA}/nfts?limit=200`
   paginated via `next` → per-token traits. 5000 ComboX tokens = ~25 pages ≈ 5s.
...
3. **Match** — `GET /listings/collection/claystonkz/best?limit=50` (needs
...
- Pre-reveal tick on claystonkz (target): silent, exit 0. ✓
- ComboX (revealed stand-in): ranked 5000 in seconds; top-5 = the 1/1 mythics
  (score 45.98b). ✓
- Live claystonkz #1081 @ 0.0135 ETH: `build_buy_tx` + `dry_run_ok` integrated
  → "SUCCESS (would fill)". ✓
- ComboX listed entirely on USDG rail → target filter correctly yielded none
...
- ComboX slug is `combox-nft`, not `combox` (combox = a different collection) —
...
- **Approval flow that worked for arming real spends**: workspace_propose one
  ...
  workspace_decide("yes") → execute. Emad answers "yes" once and expects both
  items in a multi-item message to be covered — but each *distinct*
  consequence (spend vs cron change) still needs its own slate.

## Integrating a module into Mint Room (proven 8/24 — the sniper)

Mint Room = `~/Projects/rh-mint-command-center` (Next.js App Router, prod
`next start` via launchd service `com.patelai.rh-mint-room`, live on
...
1. **lib/server/<mod>.ts** — pure logic, read-only reflection of whatever the
   daemon/script writes. Read `~/.hermes/rarity/claystonkz/snipe_state.json`
...
6. **Build + restart**: `npm run build` (must succeed — the new route/page
   ...
   `launchctl kickstart -k gui/$(id -u)/com.patelai.rh-mint-room` to serve the
...
- **Python `/tmp`-style standalone status pages get rejected** for anything
  Mint Room should host — always integrate.
