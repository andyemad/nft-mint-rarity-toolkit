...
The request: Complete fix for the alpha-group ("funkari buying (N/M)")
...
## the user's format decision (asked via clarify, he chose)
Source-aware HEADLINE first. "Keep 'X buying (N/M)' with a breakdown line below"
and "only flag when it changes meaning" were BOTH options the user did not pick. Do
