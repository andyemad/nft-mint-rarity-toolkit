...
the user's bar for "parity" is the VISIBLE product, not the functional acceptance gates.
...
- `research/ANTI-CLONE-REVIEW.md` flags the home page as a COMPOSITION clone of MintGo (it matches the
  ...
  logo/name/artwork). Pending the user's call: keep the MintGo-shaped layout (what his screenshots asked for)
...
## Deploy + QA loop
- Deploy to the existing Vercel project (`mint-market-dashboard` -> `mint-market-dashboard.vercel.app`):
  `cd ~/Projects/mint-market-dashboard && vercel --prod --yes`. Re-deploy the CURRENT tree before handing
