---
name: mint-control-room
description: Use when editing the control room code or running its mint engines.
---

# the control room (mint-control-room) — repo playbook

Local Next.js 16 mint command center at `~/Projects/mint-control-room`:
...
launchd `com.the agent.rh-mint-room`; loopback-only APIs guarded by
...
See `references/reveal-sniper-controller-integration.md` for the in-app
auto-buy sniper (the test collection), live-editable daemon params, the
...
and evidence-based a Windows minting app/the control room settings → `references/opening-block-seadrop-forensics.md`.
...
- Rarity: `app/api/rarity-gallery/route.ts` is now the GENERIC scanner
  ...
  SURVIVE restarts; the /rarity page defaults to the legacy the rarity-test collection gallery unless a
  `?collection=` param or the "Saved scans" dropdown selects one — user confusion
  ("only shows raritytest") was a missing-switcher problem, not data loss.
  (`app/api/rarity/route.ts` still exists but is the older hardcoded the test collection
...
- **Rarity-scan verification recipe** (used 2026-08-23, all curl against the live
  loopback service): (1) REGRESSION ANCHOR — the rarity-test collection ground truth is rank 1 =
...
The engine natively mints from MANY wallets — the control room just never wired it up before.
...
- Prompt flow is the same as single-wallet; confirm_multi_mint prints configured phases
  then "Answer [y/N]:".
- the control room wiring: `/api/multi-mint` (GET list/id, POST action=mint|cancel) +
...
- **UX overhaul (the user's screenshot feedback)**: right rail grouped with labeled
...
## a Windows minting app parity program (2026-08-23 →)

the user bought a Windows minting app Desktop (field-tested NFT mint bot, windows-mint-app.xyz, v1.3.4) and wants
the control room at 100% parity-or-better. Consolidated plan:
`docs/plans/2026-08-23_windows-mint-app-parity-consolidation.md` (workspace task #27).
...
`references/windows-mint-app-recon.md`. UI extraction (sidebar/pages/dialogs from decompiled
Avalonia views + the read-only research-leaf pattern): `references/windows-mint-app-ui-extraction.md`.

**Execution behavior spec (2026-08-23, waves 7–15 prerequisite reading):**
`docs/research/windows-mint-app-parity-spec.md` (192 lines) — behavior-only paraphrase from the
decompiled C# v1.3.4 (`/tmp/windows-mint-app-source`), every claim cited to `File.cs#Member`,
...
- **Clean-room rule:** mirror BEHAVIOR only (schemas/flows from strings + DB + docs);
  never decompile-copy a Windows minting app code, assets, or branding.
...
- **Workflow lesson:** when the user drops a large new workstream mid-session, he wants the
...
- **Wave 14**: `lib/server/rpc-failover.ts` (sequential multi-endpoint failover per spec §4.1, per-endpoint min-spacing 1000/rps rate limit, ETH_MINTS_ENDPOINTS=[eth.drpc.org,1rpc.io/eth], RH single) + `proxy-pool.ts` (PROXY_URLS env rotator). L3 a Windows minting app-app RPC fix left external (diagnosing a Windows minting app's own endpoint hit Wi-Fi split-proxy 127.0.0.1:8887 — separate, outside repo).
...
- **Reveal Sniper module (2026-08-24, live):** `/sniper` page + `/api/sniper/status`
  + `/api/sniper/params` + `lib/server/sniper.ts`. Mirrors the armed the test collection
  reveal-auto-buy daemon (sniper/clay_sniper.py) — status (pid, balance, buys
  fired), guardrails table, editable params, buy history. READ-ONLY surface:
  reflects the daemon's state file `~/.hermes/rarity/testcollection/snipe_state.json`,
...
- **Trigger watcher (wave 11, 2026-08-24, COMMITTED b1d0a4b):** `lib/server/trigger-watch.ts`
  + `tests/trigger-watch.test.ts` — a Windows minting app parity §1.13/§3 pending-tx watch. Pure
 ...
 Fixed left sidebar (~232px): brand panel "THE CONTROL ROOM", sections CORE (Dashboard /,
 ...
 references/windows-mint-app-ui-extraction.md), absorbing AdvancedTaskPanel modes into an
...
- **Stale launchd server breaks the UI silently (hit 2026-08-24, the user-visible)**:
  ...
  boot-vs-build time, not mtime). Symptom: the user opens the control room after a wave
  commit and reports "UI isn't showing up nicely" — unstyled/broken render.
  Protocol: after ANY `npm run build`, run
  `launchctl kickstart -k gui/$(id -u)/com.the agent.rh-mint-room`, then curl one
...
- **THE USER HARD RULE — there must be NO right-rail "side" panels (told repeatedly,
...
- **UI parity program (2026-08-23 →)**: the user wants a Windows minting app's DESKTOP UI rebuilt as
  the control room's browser app — sidebar shell + per-view pages, not just engine parity.
  Extracted structure: `docs/research/windows-mint-app-ui-parity-spec.md` (sidebar sections
...
- **Parallel research fan-out works well for decompiled-source recon** (2026-08-23):
  ...
  waves single-owner. Details in `references/windows-mint-app-ui-extraction.md`.
- When the user redirects mid-wave ("use multiple agents", "integrate the two"), he is
...
- **Sticky headers break Playwright auto-scroll clicks**: Playwright scrolls a
  ...
  `next start` on :3000 from the user's own dev session serves STALE builds — run
...
- **Mobile shell click-shield bug (UI wave B E2E failure, fixed 80d1ead)**:
  ...
  build — the user's own long-lived `next start` on :3000 serves stale code.
...
- **Time-based watch/poll-loop tests: don't let the ticking clock start already above
 ...
 - Seven pages built via parallel leaves + parent fixes: Activity (+ /api/activity,
   load-once + manual Refresh per a Windows minting app ActivityLogView — NO polling), Transfers
 ...
 - CLOSED 2026-08-24 (commit 98c0aa9) via parent salvage — the wave-D fixes were
   ...
   desktop+mobile — then `launchctl kickstart -k …com.the agent.rh-mint-room` and
   curl fresh assets 200 before reporting anything visible.

## Multi-chain SeaDrop support (2026-08-24, committed 98c0aa9)

The request: Whole-chain expansion is a the control room architect change:
...
the user feedback after wave D: ethereum/polygon chain selectors didn't load; why is
...
- **Multi-wallet mint clarity (the user: too confusing).** The original panel used
...
The durable UX bar for the control room's PRIMARY mint flow was set here, and it is blunt:
a non-technical user who wants to mint across several wallets must not have to understand
the machinery. Pasting an OpenSea URL and picking "multi-wallet mint" should be the whole
interaction. Fear of jargon overrides correctness:
a technically-correct but wall-of-technology UI reads as "you need to be technical," and the user
...
- **Power-user plumbing NEVER lives on the Dashboard.** The raw ABI "Custom mint task
  ...
  Manual calldata / selector / ABI fields the user screenshotted as "I can't understand all this"
...
- Gate = every wave gate: full vitest + `npx tsc --noEmit` + build, then
  `launchctl kickstart -k gui/$(id -u)/com.the agent.rh-mint-room` and curl "/" 200 before
  telling the user it's live.

## Live-run error hardening (2026-08-24, commit 16f38d1) — three the user-visible failures

the user ran the app for real and hit three failures; all fixed. When the user reports "several
...
1. **"Live execution is server-locked. Restart the control room with MINT_ROOM_LIVE=1"
   ...
   the error copy to "Arm live execution in the control room (Settings → Safety)". **The UI then
   ...
   (~0.02 ETH fee ceiling, 15 min). the user is non-technical: NEVER deliver a fix that
...
2. **Funding shortfall → terminal "[INPUT] Top up … press Enter; s to skip" →
   ...
   (price 0) but every tx still pays a NETWORK FEE from the minting wallet; the user's
...
5. **the user comprehension bar (recurring, strongest-yet signal 8/24):** when he says
...
The request: Free mint ≠ free gas — the mint engine's upfront funding check was making
...
- **The engine requires each wallet to AFFORD worst case `gasLimit × maxFeePerGas`
  ...
  worst-case number — not the real few-wei fee — is what drives the top-up. the user saw the
...
- the comprehension bar: when a fee or cost is challenged as absurd, inspect the ENGINE's worst-case funding requirement (gas × maxFee + value), fix it with a `.env` config knob in the runtime config, then align any matching UI threshold —
  don't just relabel the copy.

## Wave F — Wallets ops cards + persistent history log (2026-08-24, committed a3b17c5)

A persistent complaint on the wallets page set this wave's requirement: the page made no
sense without a history log of what had already run, and the fresh-wallet distribute /
consolidate actions were invisible. The distribute/consolidate BACKEND
...
what ran. UX principle that generalizes: the user reads pages as walls of numbered
...
the user armed the dev loop LIVE this day ("arm live, do everything without asking me,
...
kickstart + live loopback curl). the user wants THIS fan-out + unlazy pattern for the
