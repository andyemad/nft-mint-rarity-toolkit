...
the user reports errors with `web.gem.elixir.sh/uploads/….png` screenshots. The two
...
**OpenSea SIWE 429 skip**, neither of which is a funding problem. the user tops up,
...
the user "nothing worked". `transactionHashes[]` in the session JSON records every
tx that left.

## F. the user wants these bugs to "never recur" — pin them with regression tests
When the user says "make sure these errors don't occur again", the answer is NOT a
...
Tell the user to hard-reload (Cmd+Shift+R) to get the new client bundle.
...
as live spend needing the user's explicit OK. Also remember the engine reads `.env`
...
tell the user exactly which wallet received them and what it cost — don't bury it.
...
2. **Atomic-write ENOENT leaking raw paths into UI**: tmp-file + rename raced
   during server restarts; error text with `/Users/<name>…` paths rendered verbatim.
   Fix: retry-once on rename failure AND sanitize all worker errors
   (`<local path>`) before persisting to the queue record.
Also UI rule the user enforced: finished queues (canceled/expired/failed/confirmed/
...
~0.00058 ETH/wallet and every ~0.0001-funded mint wallet failed. the user's rule:
...
the number before telling the user to top up — the price term may be the story.
...
the OpenSea engine dies. the user hit this on Doge Brokers (0x5Ef5…F3B) and called
...
steps above; ask the user's OK per spend before broadcasting N paid mints.
