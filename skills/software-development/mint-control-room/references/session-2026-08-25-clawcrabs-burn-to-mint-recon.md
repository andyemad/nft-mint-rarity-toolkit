...
direct-contract detector in the control room (`direct-detect.ts`) does NOT fit this
...
2. Inspect the CA with the control room `/api/direct-mint?inspect=1&contract=…`
...
5. Read state with eth_call against `https://rpc.mainnet.chain.robinhood.com`
   using keccak selectors from `eth_utils` (the sniper project venv has eth-utils).
