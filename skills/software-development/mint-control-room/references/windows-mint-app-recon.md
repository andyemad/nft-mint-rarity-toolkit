# a Windows minting app Desktop — recon record (2026-08-23)

Everything a future session needs to continue the the control room parity program without
re-mining the app. Clean-room: behavior and schemas only, no proprietary code copied.

## What it is
- a Windows minting app Desktop v1.3.4 — .NET 10 + Avalonia (ShadUI) macOS app, arm64, adhoc-signed.
  NFT minting automation + wallet management. Vendor: windows-mint-app.xyz (docs:
  windows-mint-app.gitbook.io/windows-mint-app-docs, dashboard: windows-mint-app.xyz/dashboard). Also ships a
  Discord "Bot" product (monitors/tweets/alerts) — separate, not part of parity scope.
- Install: extracted from `~/Downloads/work/a Windows minting app-osx-arm64-Setup.pkg` → copied to
  `/Applications/a Windows minting app.app`, quarantine cleared (`xattr -cr`). Gatekeeper still shows
  spctl "rejected" (adhoc signature) but the app launches fine.
- Licensed: Pro Access trial, HWID-locked, 1 reset/7 days. Key stored
  `~/.hermes/secrets/windows-mint-app_license` (chmod 600, never echo). Discord account "<name>"
  linked; wallet NOT linked in their dashboard.
- App data: `~/Library/Application Support/a Windows minting app/` — `windows-mint-app.db` (SQLite, WAL),
  `license.key`, `logs/windows-mint-app-YYYYMMDD.log`, `ImageCache/`.
...
## Data model (23 tables, from live windows-mint-app.db)
...
10. a Windows minting app gaps we exploit (parity-OR-better): no Robinhood-chain preset, no rarity
    intelligence, no deal/snipe ranking, no reveal watching — the control room already has
...
## the control room mapping (what to build where)
