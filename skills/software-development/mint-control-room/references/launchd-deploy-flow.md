# Deploying the control room code changes — launchd production service

Session-verified 2026-08-24 while adding `DELETE /api/tasks`.

## The setup
Port 3000 is owned by launchd label `com.the agent.rh-mint-room`:
- plist: `~/Library/LaunchAgents/com.the agent.rh-mint-room.plist`
- KeepAlive=true, RunAtLoad=true, ThrottleInterval=10
- runs PRODUCTION: `node .../next/dist/bin/next start -H 127.0.0.1 -p 3000`
  (NODE_ENV=production), workdir `~/Projects/mint-control-room`
