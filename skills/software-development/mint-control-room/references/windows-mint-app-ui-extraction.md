# a Windows minting app UI extraction (2026-08-23) — research-leaf findings

Raw consolidated findings from the three read-only research leaves over
`/tmp/windows-mint-app-src` decompiled Avalonia views. Condensed for the browser-rebuild
waves; the build target lives in `docs/research/windows-mint-app-ui-parity-spec.md`.
...
- LoginWindow exists but NOT ported: the control room is loopback-local, no auth gate.
...
View resolution via global ViewLocator (App.cs:769). NOTE vs the control room's shell:
a Windows minting app has NO Rarity/OpenSea-Checker/NftManager sidebar entries and NO
CORE/MODULES/MONEY/SYSTEM grouping — our sections are an adaptation; RPC and
Proxy managers are a Windows minting app pages we haven't stubbed yet (candidates for SYSTEM).
