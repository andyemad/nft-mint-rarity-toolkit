...
the user pasted a screenshot of Gem's trait filter UI and asked for the same in
the control room. Pattern for "add filtering like this <screenshot>":
...
Lesson: when a the control room route needs the OpenSea key, ALWAYS go through
...
- **Turbopack dev servers cache compiled routes**; `.next/server/.../route.js`
  stayed stale after edits. the control room runs under launchd `next start` (PROD) —
...
- **Orphaned next-server processes squat on :3000** (respawned children with
  ...
  your fix didn't work. The launchd job is `com.the agent.rh-mint-room`
...
3. **Smooth loading on transient errors**: user hit "Contract did not report an
   ...
   which spun "attempt 3 of 3" on permanently-broken contracts (the user's
   screenshot showed exactly this). Permanent 4xx = fail fast with the real
   server message.

## 4b. the rarity-test collection scraped as default (later 8/25)

The request: The `/rarity` page used to auto-load the legacy the rarity-test collection
gallery when no `collection=` param was present. Removed:

- API: no-collection GET on `/api/rarity-gallery` now returns
  `{ error: "No collection selected." }` (400). The raritytest_scores/images read
...
- General rule the user enforced: the tool has NO opinionated default collection;
...
the sniper project, not the control room → `references/reveal-sniper-multicollection.md`.
...
2. the user's tab caches old bundles — tell him Cmd+Shift+R before concluding the
   feature is broken.
...
the user hit "Could not read tokenURI from this contract." on Farting Unicorn and
...
- the user's bar: "this error should never come up again" = fix the CLASS, not the
