# Reveal Sniper — the control room integration (2026-08-24)
...
- API `app/api/sniper/status/route.ts` -> `lib/server/sniper.ts`. Loopback-guarded
  ...
  across ALL collections; the old per-collection `testcollection/` path was a v1 relic),
...
- **PATH-DRIFT PITFALL (hit 8/25):** after the daemon went multi-collection
  (v2), its params/state moved from `~/.hermes/rarity/testcollection/{params,state}.json`
  to the global `snipe_params.json` / `snipe_state.json`. The the control room page kept
...
- The the control room API mirrors the same bounds (belt + suspenders). Example clamps
...
The long-standing the sniper project belief "RH Seaport fulfillAdvancedOrder reverts
49/49, root cause unresolved" was WRONG about the root cause. Session 2026-08-24
proved a working fill against a live the test collection order:
...
- **ETH vs USDG listings (decimals matter):** the test collection lists in native ETH
  (`decimals:18`); other collections on RH (e.g. the rarity-test collection) list in USDG
...
- After `npm run build`, `launchctl kickstart -k gui/$(id -u)/com.the agent.rh-mint-room`,
...
The request: Two durable patterns came out of it:
...
- **Parse the daemon's config as the UI's source of truth.**
  ...
  the control room changes and the UI can never drift. Do NOT duplicate a watched-list
