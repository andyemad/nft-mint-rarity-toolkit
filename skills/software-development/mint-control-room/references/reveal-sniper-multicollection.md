# Multi-collection reveal sniper (the sniper project) — v2, 8/25

`~/Projects/sniper/clay_sniper.py` was rebuilt from single-collection
(the test collection) into a parallel multi-collection reveal sniper after it lost the
...
- Daemon detaches from its wrapper — a background-process "exited" notice is
  ...
  (`~/Projects/sniper/clay_sniper.log`) before restarting.
...
## the user's expectations for this tool (learned 8/24–8/25)
