...
The request: (smart-flip-tools.com). Source recon:
...
any code, per the a Windows minting app parity precedent):
...
IRC on fill / stop-trip / cap hit. Clean-room rule as a Windows minting app: public
landing/docs behavior only.

## Status at plan write (8/26)
DRAFT — awaiting the user's go for wave A. Open questions: (1) wrap ~0.1 RH ETH
from bot wallet for WETH or separate funding; (2) first live test collection
(the test collection / CACHE FLOW / his pick); (3) trait bidder first target = Clay
traits post-reveal.
