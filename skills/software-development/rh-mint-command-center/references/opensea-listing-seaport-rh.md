...
signed and posted to OpenSea from Emad's OG wallet and verified via the
...
- **Fees baked into consideration items** (not seaport-js `fees:` option):
  - OpenSea fee is EXACTLY 100 bps (1%) to
    ...
    anything but 100.
  - Creator royalty is REQUIRED (e.g. claystonkz: 690 bps to
...
cancelled by Emad on 8/24. #3309 is unlisted. Lesson for future rehearsals:
confirm keep/cancel with Emad immediately after any live post, and prefer
listing at a price he'd tolerate if it accidentally stays live.

## API-route bugs fixed during first REAL UI use (8/24 evening)

Emad's first live use surfaced three route-level bugs the standalone tests
