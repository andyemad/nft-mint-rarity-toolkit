...
on Robinhood chain. Emad drives these one-click; he wants them **fast** and to
...
Emad rightly calls that absurd ("should be able to mint with a penny"). Pin
...
Emad reported "distributor wasn't working" / "Distributed to 0/13 wallets". Ops log
...
- Restart: `launchctl kickstart -k gui/$(id -u)/com.patelai.rh-mint-room`; then
  `curl -s -o /dev/null -w "%{http_code}\n" http://127.0.0.1:3000/wallets`.
- Emad is on a browser tab — tell him to hard-reload (Cmd+Shift+R) to pick up a new
...
**3001** (something re-squatted 3000 within seconds), and Emad's tab pointed at
the old port. Always end a dev-server recovery by telling Emad which port the
app is actually on.
...
3. The pre-transfer defensive `ownerOf` check blocked everything on gated
   contracts: Clay StonKz reverts EVERY ownerOf with custom selector
