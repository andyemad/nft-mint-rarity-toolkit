...
Extends the global-params model in `reveal-sniper-mint-room-integration.md`.
Emad's ask: each collection gets its OWN parameters, edited through a clean
per-collection GUI in Mint Room instead of one shared caps file.
...
`load_params("claystonkz")` against a temp per-col file to prove resolution +
