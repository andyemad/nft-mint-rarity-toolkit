# Reveal Sniper — Mint Room integration (8/24; GENERIC since 8/25)

The multi-collection reveal auto-sniper lives in
`~/Projects/bunker-snipe/clay_sniper.py` (decoupled Python daemon) and is
surfaced inside Mint Room as a native page + loopback API. Pattern is reusable
for any guarded on-chain auto-buy surfaced through Mint Room.
...
- Mint Room = read/control surface only. `app/sniper/page.tsx` + CSS module,
...
1. **Params file is the single source of truth**: `~/.hermes/rarity/snipe_params.json`
   ...
   collections; the old `claystonkz/params.json` path is a v1 relic and pointing
...
## Add-route conventions in Mint Room
...
## Reveal-day latency upgrades (8/24, rehearsed vs ComboX)
...
3. **Tighter cadence via params, not code.** `poll_secs` is already a live
   param — drop it through the Mint Room panel for reveal day instead of
...
revealed collection (ComboX) via importlib override, run `race_rank_sources()`,
time it. Verify the winner-timeout fix by checking elapsed time (~4s), not
just success.

## Post-reveal rank pipeline in Mint Room API routes (8/25, live-verified on claystonkz)
...
### Mint Room server lifecycle PITFALLS (cost an hour)
- launchd `com.patelai.rh-mint-room` runs **`next start` (production build)**,
  NOT dev. Editing `app/**` source does NOTHING on port 3000 until
  `npx next build` + `launchctl kickstart -k gui/$(id -u)/com.patelai.rh-mint-room`.
...
Session detail for the 8/25 live reveal (claystonkz): see
...
daemon load_params(name) and Mint Room clampColParams. Daemon counts col_buys
from state buys where b["collection"]==name for per-col max_buys.

Mint Room: /api/sniper/params GET → {global, collections:{name:{params(effective), overrides[]}}};
...
When Emad challenges a zero-buy reveal ("did you fuck it up again"), do NOT reassure and do NOT accept invented guilt — run this real audit. It answers definitively whether the sniper missed anything within caps:
...
After the claystonkz reveal landed slower than OpenSea's own indexing (user was
...
- **Global paths are the ONLY correct ones.** The v2 multi-collection daemon
  ...
  `claystonkz/{params,state}.json` — single-collection relics — so cap edits
...
sniping). Mint Room GUI counterpart (per-collection cards + wallet-level caps
...
kill <pid> → background terminal from ~/Projects/bunker-snipe:
...
The venv path is ~/Projects/bunker-snipe/.venv/bin/python (eth_account); ~/.venv
does not exist.
