...
`npx wrangler d1 execute mint-field-guide --remote --command "DELETE FROM minute_bucket_minters; DELETE FROM minute_buckets;"`
...
```bash
cd ~/Projects/mint-field-guide
npx tsc --noEmit && npx eslint src/worker src/lib/indexer
npx wrangler d1 migrations apply mint-field-guide --remote   # for new migrations
npx wrangler deploy
curl -s -m 120 "https://mint-field-guide-indexer.mintfieldguide.workers.dev/run"
curl -s "https://mint-field-guide-indexer.mintfieldguide.workers.dev/mints"   # expect mixed chains
curl -s "https://mint-field-guide-indexer.mintfieldguide.workers.dev/stats?hours=24"  # expect sane mintCounts
```
