...
897 tests, test post returned HTTP 204. Webhook "eCalm Suites" → Calm #🦄alpha.
...
npx wrangler d1 execute mint-field-guide --remote --file migrations/0014_alpha_alert_state.sql
npx wrangler deploy
curl -s https://mint-field-guide-indexer.mintfieldguide.workers.dev/run \
...
- **Verify the webhook's target channel BEFORE wiring** — GET the webhook URL
  (read-only, returns name/channel_id/guild_id). Emad created it pointed at the wrong
