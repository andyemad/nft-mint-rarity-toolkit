...
Three parallel opencode agents audited eCalm Suites (mint-field-guide) and Mint Room (rh-mint-command-center). Full reports live in the repos; this file holds the durable, session-independent findings a future session should know before touching either codebase.

## Report locations
- `~/Projects/mint-field-guide/.hermes/research/mfg-audit.md` (177 lines)
- `~/Projects/rh-mint-command-center/docs/research/mint-room-audit.md` (216 lines)
- `~/Projects/research-dossier/portfolio-review-mint-tools.md` (100 lines, the strategy verdict)

## mint-field-guide (eCalm Suites) — machine-verified state
...
- ROADMAP.md is stale in ≥4 places (items marked open that shipped; decisions marked waiting-on-Emad that were answered). START-HERE.md wins every conflict per its own header.
...
## Mint Room (rh-mint-command-center) — critical defects found by audit
...
- Keep B (Mint Room) PRIVATE permanently — loopback + local key is its moat. Its needed test is internal: instrument realized P&L per mint.
...
- Archive: mintgo-intel (delete + Vercel project), mint-radar, ecalm-mint-sniper, rh-mint-bot (audit its wallets/ dir for loose keys first). Demote rarity-engine to "B's batch data layer".
