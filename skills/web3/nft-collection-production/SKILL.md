...
Say this plainly up front — the user has been burned by hype-then-deflate framing:
...
- Keep verdicts in net$ vs time/risk terms, give the honest race/cost, and
  let the user decide "worth it". Do not over-promise.
- IP: NEVER 1:1 clone a copyrighted meme character (Pepe the Frog = Matt
  ...
  ORIGINAL character and a different meme fusion. Flag takedown risk when a live collection is pointed at as a copy target.
- **Deliver the honest gate ONCE, then BUILD — don't re-lecture.** The no-buyer-room math is already understood; "make it work" means build the thing that creates demand pressure, not re-explain why distribution is the ceiling. Saying "you can't make money at 24 supply / you need a room"
...
- **Anonymity is broken the moment you FUND the deploy wallet from any wallet
  linked to the user.** A new wallet is not anonymous on its own — it becomes
  ...
  wallet MUST be funded from an unrelated source the user supplies: an exchange
  ...
  that source from the user, and never use the bot wallet/any owned wallet to seed
  an "anonymous" mint.
...
When the user asks "what can YOU come up with" (e.g. after Claude runs out of
...
- **Genuinely aesthetic / has PRESENCE** (texture, mood, light, grain, depth).
  the user rejected flat-vector cap-art as "this sucks" and rated the ORIGINAL
...
illustration.** the user rejected three such passes—including a neon-on-black
...
- **Trait-system anatomy (the "bits and parts", not whole-token art)?** the user
...
- the user rejects static mockups — the deliverable is the working generator +
...
## Mint path (when the user says go)
...
- **the user's preferred paid-mint mechanic: AGENT-GATED mint.** `mint()` reverts
...
- **When the ask is a high-pressure mint page, build it — see `references/fomo-mint-page.md`** for the six-trigger stack (live mint feed,
...
- **Sizing supply / free-wave / price to hit a revenue target — see
  `references/mint-pricing-ladder.md`.** the user's shape: supply in the thousands
...
- **The final cycle-jackpot economy + local-EVM verification — see
  `references/cycle-jackpot-economy.md`.** the user's locked shape (2026-08-20, supersedes
...
- **Decaying-price (time-decay) mint — see `references/decaying-price-mint.md`.**
  the user's non-jackpot mechanic of choice (2026-08-20): currentPrice = start −
...
- **Whitelist/wallet-collection intake (IRC, one wallet per message):** when
  the user pastes a stream of wallets one per message for a whitelist cap, write
...
- **When the user reverses a plan, the NEWEST spec supersedes prior handoffs.** Before
...
5. Distribution is the actual product: Twitter/X channel, meme fusion pull,
   momentum window. the user's standing rule: never list a paid mint below
...
When the user drops a live collection/X account and wants the same play, run
