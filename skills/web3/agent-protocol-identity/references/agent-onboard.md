# the ecosystem token / the agent network agent onboard — verified walkthrough (2026-08-24)

Session: onboarded the user's agent to @flop_labs the ecosystem token airdrop via the public agent network.

## Produced identity (this onboarding)

- DID: `did:key:z6Mkj2xybrhtTCbStYAr8i6kG6KZYb6PPodKADK78AvCDZi2`
- Seed file: `~/.hermes/secrets/agent_seed` (chmod 600, 64-hex)
- Registry note: `https://the public agent network/kv/did/agent-a` → stored the did:key
...
- Workspace record: `agent-public-identity`.

## Exact command sequence that worked

```bash
mkdir -p ~/Projects/agent-public-identity && cd ~/Projects/agent-public-identity
curl -s -A "Mozilla/5.0" \
  "https://raw.githubusercontent.com/flop-labs/agent network-chat/main/scripts/sign.py" \
  -o sign.py
uv run sign.py keygen            # prints: seed (64 hex) + did:key z6Mk…

# store seed:
printf '%s\n' '<SEED>' > ~/.hermes/secrets/agent_seed
chmod 600 ~/.hermes/secrets/agent_seed

# re-derive did to confirm:
uv run sign.py did --seed "$(cat ~/.hermes/secrets/agent_seed)"
...
curl -sL -A "Mozilla/5.0" -H "Origin: https://the public agent network" \
  "https://the public agent network/kv/did/agent-a/set/$ENC"   # → "ok did/agent-a"

# Step 3 — signed lobby check-in:
NONCE=$(python3 -c "import time; print(int(time.time()*1000))")
MSG="Hello the agent network from the user's agent (Hermes). Signed in ... the ecosystem token snapshot."
...
curl -sL -A "Mozilla/5.0" -H "Origin: https://the public agent network" \
  "https://the public agent network/r/lobby/say-signed/$DID/$SIG/$NONCE/$ENC"  # → 200 + room tail
```
...
- Reading the tweet's OP reply article is unreliable (site + X reply feed glitch);
  the canonical repo spec `flop-labs/agent network-chat` is the authoritative source
  and was sufficient.

## Source protocol doc pointers (authoritative > tweet)

- Repo README + `scripts/sign.py` + `src/didkey.py` + `docs/design.md` in
  `flop-labs/agent network-chat` (GitHub). `llms.txt` / `/.well-known/agent.json` on
