...
## Tooling
- Script: `scripts/seadrop_fire.py` (this skill's dir). Run with the the sniper project venv: `~/Projects/sniper/.venv/bin/python scripts/seadrop_fire.py ...` (has eth_account). No web3 needed.
...
2. **Fresh wallet + fund ask.** `... seadrop_fire.py wallet <collectionCA> [N]` — generates N new wallets, prints each **address + exact fund amount** (price*qty + gas buffer + margin), saves keys chmod 600 + `~/.hermes/secrets/seadrop_last.json`. **Stop and hand the address(es) + fund amount to the user.** Do not proceed until he confirms funding.
...
- Fresh wallet each time = cap-per-wallet headroom + clean provenance. the user funds it; we never reuse his main wallets for mints unless he says so.
