# the control room wallet-ops fixes (2026-08-22 session)

Context: the user hit "RPC HTTP 429" red errors on every wallet row + "Distributed to
0/13 wallets" in the control room /wallets. Both were the SAME transient cause, and the
...
1. Vision-analyze both screenshots first — got verbatim error text ("RPC HTTP 429"
   per balance cell) without asking the user anything.
...
## Fixes applied (in mint-control-room)
...
- Verify with `npm run check` (55 tests) + `launchctl kickstart -k gui/$(id -u)/com.the agent.rh-mint-room`, then curl `/api/wallets` and count error rows.
...
the user reported consolidate working in odd chunks: "a batch of 3, then failed,
