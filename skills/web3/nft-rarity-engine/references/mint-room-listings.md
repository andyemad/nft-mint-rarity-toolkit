# Mint Room floor + listings feed (added 2026-08-22)
...
- Necrochrome (0x1e64…c9f8): 87 listing rows → 75 unique tokens listed, floor 0.000188 ETH. Both stored keys work for listings (`~/.hermes/secrets/opensea_key`, len 32; the launchd plist key also valid).
- ComboX (0x512f…d55c): 535 listed of 5000; cheapest at-floor: #3174 (rank 550), #1857 (rank 1423), #486 (rank 1533), #2483 (rank 1540), #4284 (rank 2253) — all at 0.000094.
- Clay StonKz best-listings endpoint paginates (>100 rows); /best returned exactly 100 with next cursor.
...
```
curl -s "http://127.0.0.1:3000/api/floor?collection=os:necrochrome"
# expect {"slug":"necrochrome","floor":"0.000188","listedCount":75,...}
...
# expect slug combox-888, floor 0.000094, some tokens with price
python3 - <<'EOF'
import json
...
- **UPDATED 2026-08-22 (Emad directive — supersedes the paragraph below):**
...
- Verified: Necrochrome pre-reveal → 75 listed @ 0.000188 floor; Clay StonKz → 341 listed @ 0.01644.

## Publish outcome (2026-08-22 evening)

Both repos went public, then **flipped back to PRIVATE the same evening** after
Emad's friend cloned them ("my friend cloned them already so you can put it
...
- `github.com/andyemad/rh-mint-command-center` — osnm-z engine VENDORED into the tree (submodule pointer removed via `git rm --cached` + rsync from a fresh clone of zunmax/osnm-z, excluding `target/` [2.1GB Rust build cache] and `.git`; local ~200 lines of submodule modifications captured that were never pushed upstream; upstream MIT LICENSE kept in-tree + README attribution). README with real setup + MIT LICENSE added. The publish commit also carried smart-wallets endpoints and probe scripts (`scripts/buy.py`, `thepool_reveal_watch.py`, etc.).
- `github.com/andyemad/mint-field-guide` — flipped private→public→private with `gh repo edit --visibility ... --accept-visibility-change-consequences` (the consequences flag is required non-interactively). `/.hermes/` scratch dir gitignored and committed BEFORE flipping so untracked junk can never push.
