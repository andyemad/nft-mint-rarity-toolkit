...
Class playbook for: computing collection-wide rarity ranks that MATCH OPENSEA EXACTLY, detecting reveals, sweeping metadata/images, and rendering a visual gallery. Proven on the rarity-test collection (5000), the test collection (6969), THE TEST COLLECTION (2000) — all Robinhood Chain.
...
Naive trait-frequency scoring produced visible discrepancies the user noticed vs OpenSea (#2401 showed 71 vs true 353). Info-content matched 5/5 sampled tokens exactly and 0/5000 mismatches in regression. Always use info-content.

## Pipeline (per collection)

1. **Resolve input** — accept raw CA, full OpenSea URL (any locale/`/activity` suffix/query junk), or `os:slug`. Slug→CA via OpenSea page scrape of `push(`-embedded urql JSON; prefer `chain.identifier == "robinhood"` when multi-chain. Reusable code: `~/Projects/mint-control-room/lib/server/opensea-resolve.ts` (`parseCollectionInput`, `resolveOpenSeaSlug`).
...
3. **Sweep — SPEED-CRITICAL source order.** Primary: OpenSea API v2 **paged list endpoint** `/chain/<chain>/contract/<ca>/nfts?limit=200` + `next` cursor. It includes FULL TRAITS and image_url per token (per-token single-NFT calls do too, but burst-429 badly). `limit=200` measures ~0.2s/page → ~5s for 5000 tokens across 25 pages (08-24, the test collection/the rarity-test collection) — meaningfully faster than `limit=50` (~3.5s/600 → 35s/5000); use 200. Never use the per-token endpoint as the primary sweep (43 MINUTES for 5000 due to sustained-burst 429s). Fallback (for the window right after a reveal before OpenSea indexes it): on-chain tokenURI at 96-way RPC concurrency (~46s for 5000, measured) + IPFS metadata via rotating-gateway worker pool. Expect ~1% RPC failures; rank what you get, report missing count.
...
- **Listings feed (floor + per-token prices):** `/api/v2/listings/collection/<slug>/all?limit=50&next=...` pages every active listing (reduce to cheapest-per-token client-side); `/best` variant returns only each token's cheapest order but caps at ~100 rows and does NOT page reliably — use `/all` + own reduction when you need full book coverage. Listings are SLUG-keyed only: a raw CA must first resolve to a slug via `/api/v2/chain/robinhood/contract/<ca>` → `.collection`. Verified live: the test collection 87 rows → 75 listed tokens, floor 0.000188; the rarity-test collection 535 listed. Reusable code: `~/Projects/mint-control-room/lib/server/opensea-listings.ts`.
...
- One transiently-empty token in your trait dump skews EVERY count by -1 across its trait types (found: the rarity-test collection #3787). Sanity check: per-trait-type sums must equal total supply; find and refetch any empty-trait token before ranking.
- Empty URI ≠ missing token: probe id boundaries before assuming sweep range (THE TEST COLLECTION ids are exactly 1..2000).
...
- `references/control-room-rarity.md` — where the live code lives in the control room (endpoints, pages, data files, service restart command). NOTE: after the speed fix, the sweep primary in `app/api/rarity-gallery/route.ts` is the paged list endpoint; measured 5000-token scan = 10.5s fresh / 45ms cached.
- `references/control-room-listings.md` — the floor/listings feed added 2026-08-22 (`lib/server/opensea-listings.ts`, `/api/floor`, per-token price merge on the gallery, /rarity UI badges), verified numbers, API quirks (slug-keyed listings, python-UA-403, /best vs /all, top-level contract-endpoint shape), and the saved-scans switcher (now: every scanned collection persists pre-reveal included).
- `references/control-room-wallet-ops.md` — /wallets 429-outage post-mortem (balance errors + 0/13 distribute from one transient RPC throttle), the retry/backoff + single-nonce fixes now in rpc.ts and wallets/route.ts, and verification steps.
- `references/control-room-smart-wallets.md` — smart wallet tracker (GuarEmperor degen list, panel 07, /api/smart-wallets): the rawRpcCall-returns-object-not-JSON hang and the getLogs topic1-vs-topic2 incoming/outgoing trap, both with symptoms; measured scan timings; data-source refresh flow.
- `references/seaport-buy-path.md` — the WORKING programmatic buy primitive (verified via eth_call simulation on a live THE POOL listing), the bunker 49/49-revert post-mortem correction (encoding was never broken — dead orders were), guardrail design, and where the code lives (`~/Projects/mint-control-room/scripts/buy.py`).
...
- Watcher pattern (cron no-agent script probing every 2m, iMessage alert on reveal): `~/.hermes/scripts/testcollection_reveal_watch.py`; generic reveal watcher with auto-sweep+rank: `~/Projects/mint-control-room/scripts/thepool_reveal_watch.py` (v2 — see post-mortem below).

## Live-editable sniper params in the control room (armed 8/24)

The guardrails must be editable FROM the UI, without touching code or restarting the
daemon — for example changing the per-buy cap from 0.01 to something else. The `/sniper` page the control room
page exposes all 7 caps as inputs; saving them must take effect immediately.

**Architecture (the durable pattern):**
- Parameters live in a JSON file (`~/.hermes/rarity/testcollection/params.json`)
...
**CRITICAL React pitfall (cost the user a bug report):** a `<input type=number>`
...
- During verification I wrote clamped test values into the live armed daemon's
  params (as part of proving the clamp path), then restored the user's approved caps.
...
User preference reinforced this session: the control room is meant to be the ALL-IN-ONE
surface for the whole mint workflow (the goal being one surface for everything)
...
verification only; the permanent home is the the control room page.
...
and the cron logged `ok` for hours (empty stdout = silent by design). the user caught
...
## Reveal-snipe economics (what to actually tell the user)
...
## Floor + listings feed in the control room (built 2026-08-22)
...
- `/rarity` UI: floor + listed count in header, price badge per card (gold = at/near floor), "Refresh listings" button, listed-at line in detail sheet. Sniper view = sort top ranks by price: rank #550/#1423/#1533 at floor on the rarity-test collection was found this way.
...
## Pre-reveal collections propagate + show listings (the user directive, 2026-08-22)
...
## Sharing the the control room codebase publicly (2026-08-22, EXECUTED — repos live)

the user opened everything under the control room plus mint-market-dashboard. Audit + publish recipe, proven end to end:
...
**Vendoring a submodule (the user's call: "what if they take that repo down?" — right instinct):**
```
...
Live results: `github.com/andyemad/mint-control-room` (public, self-contained w/ vendored engine) and `github.com/andyemad/mint-market-dashboard` (flipped public, 98 commits of history intact).

Identity note: these repos tie the NFT tooling to one GitHub account. Keeping that work separate from a personal or employer-linked identity would require a fresh account.
...
the old one. Budget one extra user "yes" for this round-trip; warn the user when the
