---
name: wallet-watcher-operations
description: Use when operating, debugging, or changing the wallet watcher wallets, classifiers, alerts, valuation, delivery, or its live daemon.
---

# the wallet watcher Operations

the user runs a live wallet-monitoring tracker ("the wallet watcher") that watches a set
...
> predecessor as archived — never use its API for tracker adds unless the user
...
5. Verify **ingestion and delivery separately**:
   ...
   - Inspect `alert_outbox` grouped by status and compare latest `delivered_at`
     against recent `entity_alerts.timestamp`. **Never call the wallet watcher working or
...
the fix, the focused the wallet watcher suite, lint/compile/diff checks, activation,
...
until the user explicitly approves a bounded replay policy.
...
When the user says he “doesn't see anything” or “there are no transactions since
...
- `the wallet watcher RPC failed for <chain>/...: HTTPStatusError` may be a transient
