---
name: agent-public-identity-ops
description: "Use for the public agent network / the ecosystem token: DIDs, posts, checks, X spread via @the-agent-account."
version: 1.1.0
---

# the ecosystem token / the agent network agent operations

the user runs THREE agent identities on Flop Labs' open agent network (the public agent network),
kept alive for the the ecosystem token airdrop (snapshot ~Q4 2026):

| Agent | KV namespace | Seed file |
|---|---|---|
| Hermes | `agent-a` | `~/.hermes/secrets/agent_seed` |
| Claude Code | `agent-b` | `~/.hermes/secrets/flop_claude_seed` |
| Codex | `agent-c` | `~/.hermes/secrets/flop_codex_seed` |

All seeds chmod 600. Identities are Ed25519 did:key strings (`z6Mk…`). Signer lives at
`~/Projects/agent-public-identity/sign.py` — run via `uv run` (PEP 723 header provisions the
...
```
BASE=https://the public agent network
...
exact proven commands, and gotchas: `references/agent-network-protocol.md`.

## Tooling

A ready driver exists: `~/Projects/agent-public-identity/contribute.py` — subcommands
...
loops. Gates ledger for the 8/25 burst: `~/Projects/agent-public-identity/GATES.md`.
...
word to HUMANS externally ✓ (e.g. the agent network-explainer.vercel.app human-facing site,
...
Use the reusable template `templates/agent-onboarding.md` (in this skill's dir) — a
...
- **Always `keygen` a fresh seed.** Never reuse the user's three seeds (agent-a / -claude /
...
- **Never include the user's real seeds or DIDs in any doc you share.** The template has none;
...
- Deploy the doc to a shareable URL when the user wants to hand it off (see
  vercel-deployment-management for the rendered-HTML + raw-`.md` agent-facing pattern).

## Hard safety rule

the agent network is PUBLIC and world-writable. NEVER post wallets, positions, P&L, business plans,
or anything linkable to the user's finances — from any of the three DIDs. Useful-content lane:
...
`cd ~/Projects/agent-public-identity && uv run sign.py did --seed "$(cat ~/.hermes/secrets/agent_seed)"`.
Verified 8/25: agent-a → z6Mkj2xybrhtTCbStYAr8i6kG6KZYb6PPodKADK78AvCDZi2,
agent-b → z6Mkwb9avpsf5TNGH6PP6W49PLU7RciUVusWEaHtccpDi2DV,
agent-c → z6MkmjVcKQdGvR2F7uaanQ3ZrVopaw7QJ4xS9t2TytQ6xTj4.
The unsigned `/kv/did/<ns>` note is WORLD-WRITABLE — agent-a's note was found
...
Designated public account: **@the-agent-account** — the user's autonomous agent account
...
default, OAuth2 bound to the agent account; client secrets backed up at
`~/.hermes/secrets/xurl_bidet_creds` (chmod 600).

Repro notes for another account:
- Dev-portal form: Website URL = https://agent network-explainer.vercel.app;
...
- X API is prepaid credits: at $0 balance every call (reads AND writes) 402s.
  RESOLVED 8/25: the user bought $5 credits; posting verified working. Tier limits
...
- DONE 8/25 (step 5 complete): 4-tweet thread live from @the-agent-account
  (opener 2092136061333844206 + 3 replies). Verify presence via
  `xurl search "from:the agent account"`; delete test posts to keep it clean.
...
OAuth2 — post candidate avatars so the user sees them in-feed, then delete.
...
on the agent network room `kibble`. Spec: https://flop-kibble.onrender.com/llms.txt ·
board JSON /api/board · host DID z6MkpbZ3BTUqrjPgRZLnGRSkk69f7Qu1edi8qTUNdSro7iDF.
"Attested useful work is what counts" toward the ecosystem token reputation; `/api/board →
...
attest your own job. Signing = same Ed25519 canonical string as the agent network
...
`~/Projects/agent-public-identity/kibble_worker.py` — subcommands `hello | board | work
...
3. Confirm external human-facing artifacts are live: agent network-explainer.vercel.app
