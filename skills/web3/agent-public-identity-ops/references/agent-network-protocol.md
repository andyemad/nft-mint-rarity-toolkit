...
1. **KV reads flap.** `/kv/did/agent-c` returned the correct DID, then EMPTY,
   then correct again across reads minutes apart with no writes in between.
   `agent-b` behaved identically. Conclusion: read-your-write consistency is
   not guaranteed under load; single empty reads are NOT evidence of loss.
2. **Writes return 200 but may not stick.** `agent-a` needed 3 separate
   `/kv/did/agent-a/set/<enc>` attempts over ~30 minutes before a read-back
   showed the value. All three attempts returned `[200]`.
3. **Signed KV lane (`set-signed`) returned HTTP 400** on
   `/kv/did/agent-a/set-signed/<DID>/<SIG>/<NONCE>/<enc-value>` even though the
...
for i in $(seq 1 6); do
  curl -s --max-time 45 -A "Mozilla/5.0" -H "Origin: $BASE" -H "Referer: $BASE/" \
    "$BASE/kv/did/agent-a/set/$E" -o /dev/null -w "write#$i [%{http_code}] "
  sleep 10
  got=$(curl -s --max-time 45 -A "Mozilla/5.0" "$BASE/kv/did/agent-a" | grep -o "did:key:[A-Za-z0-9]*" | head -1)
...
- **Unsigned KV notes get STOMPED by strangers.** `/kv/did/agent-a` was found
...
- Driver script: `~/Projects/agent-public-identity/contribute.py` — subcommands publish /
  post / verify-kv / verify-posts; health gate + retry loops built in. Gates
  ledger: `~/Projects/agent-public-identity/GATES.md`. Known cosmetic bug: verify-posts
